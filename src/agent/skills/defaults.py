...
- ideal_buy 填写“数据质量不足，不提供具体买入点”
- secondary_buy 填写“数据质量不足，不提供具体买入点”
- stop_loss 填写“数据质量不足，不提供机械止损位”
- take_profit 填写“缺乏可靠估值依据，不提供目标价”
- position_strategy.suggested_position 填写“仅监控，不提供仓位比例”
- position_strategy.entry_plan 填写“等待数据恢复与事实确认”
- position_strategy.risk_control 填写“使用投资论点证伪条件，不提供机械价格止损”
...
def get_default_trading_skill_policy(*, explicit_skill_selection: bool) -> str:
    """Return the global fact-first investment monitoring baseline."""
    return CORE_TRADING_SKILL_POLICY_ZH


def get_default_technical_skill_policy(*, explicit_skill_selection: bool) -> str:
    """Return the global fact-first technical context baseline."""
